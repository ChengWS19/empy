...
class AppResCalculator:
    ...
    def linearSearch(self, rho, time_point, field_value):
        ...
        self.parameters["freqtime"] = [time_point]

        self.res_list[-1] = rho
        self.parameters["res"] = self.res_list
        Bp_t1 = np.abs(empymod.bipole(**self.parameters))
    ...
    def GradB(self, res, time_point):
        ...
        self.res_list[-1] = res
        self.parameters["res"] = self.res_list
        Bi1 = np.abs(empymod.bipole(**self.parameters))

        self.res_list[-1] = res * 1.1
        self.parameters["res"] = self.res_list
        Bi2 = np.abs(empymod.bipole(**self.parameters))
...
def main():
    ...
    app_res_calculator = AppResCalculator(inputfile, src, rec, depth, res_list, time_channel, search_value)
    if parallelflag:
        # 获取CPU核心数
        num_cores = cpu_count()
    else:
